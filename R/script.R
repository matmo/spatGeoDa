...
# To see all shortcuts press Alt + Shift + K
...
# Need to run this line, otherwise Error message with has_devel (Could not find tools necessary to compile a package)
pkgbuild::find_rtools(debug = TRUE)
has_devel()

# Creates new package
create_package("C:/Users/jderndor/Documents/Lehre/spatGeoDa")
...
cleveland_df <- cleveland@data

usethis::use_data(cleveland, cleveland_df, overwrite = T)

#sinew::makeOxygen(cleveland_df, add_fields = "source")
# Example for documentation of Cleveland Data
#' Cleveland Home Sales (2015)
#'
#' Location and sales price of home sales in a core area of
...
url <- "https://geodacenter.github.io/data-and-lab//clev_sls_154_core/"
...
#---back up description file (data.R)--------
#' AirBnB Chicago 2015
#'
...
#'  \describe{ #'   \item{community}{name of community area}
...
#'  \describe{ #'   \item{NAME}{county name}
...
#'  \describe{ #'   \item{BLOCK}{Census block}
...
#'  @format A data frame with 205 rows and 9 variables
#'  \describe{ #'   \item{unique_id}{unique parcel id}
#'   \item{parcel}{unique parcel number}
#'   \item{x}{point latitude}
#'   \item{y}{point longitude}
#'   \item{sale_price}{price paid for the house ($)}
...
#'  \describe{ #'   \item{PST045214}{Population, 2014 estimate}
...
#'  \describe{ #'   \item{ratio}{The county’s median income divided by the state’s median income}
...
#'  \describe{ #'   \item{BirthRate}{Births per 1,000 residents}
...
#'  \describe{ #'   \item{DepEcProv}{Deprivation in economic provisioning}
...
#'  \describe{ #'   \item{FORHIS06-09}{percentage of hispanic population, not born in US}
...
#'  \describe{ #'   \item{YR}{02 to 14 (2002 to 2014)}
...
#'  \describe{ #'   \item{POLY_ID}{unique ID}
...
#'  \describe{ #'   \item{BoroCode}{Number for each of the five New York City boroughs}
...
#'  \describe{ #'   \item{AREA}{county polygon area (computed by      ArcView)}
...
usethis::use_data(airbnb, atlanta, cincinnati, cleveland, election, health_diversity, health_indicators,
                  ncovr, nepal, nyc, nyc_earnings, nyc_educ, nyc_nhood, police, overwrite = T)
